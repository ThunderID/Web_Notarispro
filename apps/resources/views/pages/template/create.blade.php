...
@section('title')
	<h4>
		@if(Request::route()->getName()=='edit.template.akta')
			<small><a class="btn btn-primary btn-sm" href="{{route('show.template.akta', ['id' => env('sandbox_id')])}}"><</a></small>
		@else
...
@section('action')
	@if(Request::route()->getName()=='edit.template.akta')
		...
		<a class="btn btn-primary" href="{{route('update.template.akta', ['id' => env('sandbox_id')])}}" role="button">Simpan</a>
	@else
